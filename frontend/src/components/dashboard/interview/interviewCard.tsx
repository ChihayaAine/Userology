...
function InterviewCard({ name, interviewerId, id, url, readableSlug }: Props) {
  ...
  const [img, setImg] = useState("/FoloUp.png");

  useEffect(() => {
    const fetchInterviewer = async () => {
      console.log('🚀 InterviewCard: Fetching interviewer with ID:', interviewerId, typeof interviewerId);
      
      const interviewer = await InterviewerService.getInterviewer(interviewerId);
      
      console.log('✅ InterviewCard: Received interviewer data:', interviewer);
      console.log('✅ InterviewCard: Interviewer image field:', interviewer?.image);
      
      if (interviewer && interviewer.image) {
        setImg(interviewer.image);
        console.log('✅ InterviewCard: Set image to:', interviewer.image);
      } else {
        console.warn('⚠️ InterviewCard: No image found, keeping default');
      }
    };
    fetchInterviewer();
